...
abstract class CLTheme {
  ...
  /// Vista dei 20 token semantici nella forma di `ShadColorScheme`
  /// (flutter-shadcn-ui), mappati sulla NOSTRA palette. Serve a copiare codice
  /// Shad e internalizzarlo con attrito minimo: `ShadTheme.of(context).colorScheme`
  /// → `CLTheme.of(context).shad`. Vedi [CLShadScheme] per il remap deliberato
  /// (Shad `secondary`/`accent` sono GRIGI → muted/accent, NON il nostro blu).
  CLShadScheme get shad => CLShadScheme(this);
...
/// Vista `ShadColorScheme`-shaped su un [CLTheme]: espone i 20 token semantici
/// di flutter-shadcn-ui con gli STESSI nomi, ma pescando i colori dalla nostra
/// palette. Obiettivo: copiare un widget Shad e sostituire solo
/// `colorScheme` → `theme.shad`, senza toccare i singoli `.token`.
///
/// Remap deliberato (differenze palette CL ↔ Shad):
/// • Shad `secondary`/`accent` sono GRIGI → mappano su `muted`/`accent` NOSTRI,
///   NON su `CLTheme.secondary` (blu brand). Per questo serve un adapter e non
///   dei getter diretti su CLTheme (collisione di nome + di ruolo).
/// • `primary` mantiene il ruolo (azione primaria) → nostro blu, non lo zinc.
/// • `*Foreground` su superfici piene forti = bianco; `selection` = primary @ .25.
class CLShadScheme {
  const CLShadScheme(this._t);
  final CLTheme _t;

  // Shad `background` (zinc) è BIANCO, non il canvas: mappa sulla superficie
  // bianca nostra (secondaryBackground). Il canvas grigio (primaryBackground)
  // non ha un corrispettivo tra i 20 token Shad.
  Color get background => _t.secondaryBackground;
  Color get foreground => _t.primaryText;
  Color get card => _t.secondaryBackground;
  Color get cardForeground => _t.primaryText;
  Color get popover => _t.secondaryBackground;
  Color get popoverForeground => _t.primaryText;
  Color get primary => _t.primary;
  Color get primaryForeground => const Color(0xFFFFFFFF);
  Color get secondary => _t.muted; // GRIGIO (Shad secondary), non il blu brand
  Color get secondaryForeground => _t.accentForeground;
  Color get muted => _t.muted;
  Color get mutedForeground => _t.mutedForeground;
  Color get accent => _t.accent;
  Color get accentForeground => _t.accentForeground;
  Color get destructive => _t.danger;
  Color get destructiveForeground => const Color(0xFFFFFFFF);
  Color get border => _t.borderColor;
  Color get input => _t.borderColor;
  Color get ring => _t.ring;
  Color get selection => _t.primary.withValues(alpha: 0.25);
}
