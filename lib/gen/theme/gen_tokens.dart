...
/// CLTheme-shaped accessor over the Gen (Shad) theme, so the migrated custom
/// widgets (GenAdaptiveShell, …) keep reading `theme.X` with minimal edits:
/// colors come from [ShadColorScheme], spacing/sizes are constants, text styles
/// map to [ShadTextTheme]. Get it with `GenTokens.of(context)`.
class GenTokens {
  GenTokens._(this._cs, this.brightness);

  factory GenTokens.of(BuildContext context) {
    final t = ShadTheme.of(context);
    return GenTokens._(t.colorScheme, t.brightness);
  }
